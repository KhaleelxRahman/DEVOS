...
async def test_waitlist_join_and_duplicate(client):
    res = await client.post("/api/v1/waitlist", json={"email": "Early@Example.com", "name": "Early"})
    assert res.status_code == 200, res.text
...
async def test_contact_submission(client):
    payload = {"name": "Dev", "email": "dev@example.com", "subject": "Hello", "message": "This is a test message."}
...
async def test_contact_honeypot_rejected(client):
    payload = {"name": "Bot", "email": "bot@example.com", "subject": "Spam", "message": "Buy things", "website": "http://spam"}
    res = await client.post("/api/v1/contact", json=payload)
...
async def test_contact_rejects_html_and_oversize(client):
    html = await client.post("/api/v1/contact", json={
        "name": "Dev", "email": "dev@example.com", "subject": "<script>alert(1)</script>", "message": "hi"})
    assert html.status_code in (400, 422)
    big = await client.post("/api/v1/contact", json={
        "name": "Dev", "email": "dev@example.com", "subject": "s", "message": "x" * 5000})
...
async def test_file_folder_create_read_save_rename_delete(client):
    ...
    res = await client.post(f"/api/v1/projects/{pid}/files/folder", json={"parent_path": "", "name": "src"}, headers=headers)
    assert res.status_code == 200, res.text

    res = await client.post(f"/api/v1/projects/{pid}/files/file",
                            json={"parent_path": "src", "name": "app.py", "content": "print(1)"}, headers=headers)
    assert res.status_code == 200, res.text
    assert res.json()["data"]["content"] == "print(1)"

    res = await client.put(f"/api/v1/projects/{pid}/files/src/app.py", json={"content": "print(2)"}, headers=headers)
    assert res.status_code == 200
    res = await client.get(f"/api/v1/projects/{pid}/files/src/app.py", headers=headers)
    assert res.json()["data"]["content"] == "print(2)"

    res = await client.post(f"/api/v1/projects/{pid}/files/rename",
                            json={"path": "src/app.py", "new_name": "main.py"}, headers=headers)
    assert res.status_code == 200, res.text
    assert res.json()["data"]["path"] == "src/main.py"

    res = await client.delete(f"/api/v1/projects/{pid}/files/src/main.py", headers=headers)
    assert res.status_code == 200
...
async def test_file_create_rejects_traversal_and_sensitive(client):
    ...
    bad = await client.post(f"/api/v1/projects/{pid}/files/file",
                            json={"parent_path": "../..", "name": "evil.txt"}, headers=headers)
    assert bad.status_code in (400, 403, 422)

    dot = await client.post(f"/api/v1/projects/{pid}/files/file",
                            json={"parent_path": "", "name": ".env"}, headers=headers)
    assert dot.status_code in (400, 403)

    res = await client.post(f"/api/v1/projects/{pid}/files/file",
                            json={"parent_path": "", "name": "exists.txt"}, headers=headers)
    assert res.status_code == 200
    dup = await client.post(f"/api/v1/projects/{pid}/files/file",
                            json={"parent_path": "", "name": "exists.txt"}, headers=headers)
...
async def test_upload_files_and_blocked_extension(client):
    ...
    res = await client.post(f"/api/v1/projects/{pid}/files/upload", files=files, headers=headers)
    assert res.status_code == 200, res.text
    assert res.json()["data"]["uploaded"] == ["hello.txt"]

    content = await client.get(f"/api/v1/projects/{pid}/files/hello.txt", headers=headers)
    assert content.json()["data"]["content"] == "hello world"

    bad = await client.post(f"/api/v1/projects/{pid}/files/upload",
...
async def test_upload_rejects_oversized(client):
    ...
    res = await client.post(f"/api/v1/projects/{pid}/files/upload",
                            files={"files": ("big.bin", big, "application/octet-stream")}, headers=headers)
    assert res.status_code == 200
...
async def test_files_cross_user_denied(client):
    ...
    await client.post(f"/api/v1/projects/{pid}/files/file",
                      json={"parent_path": "", "name": "secret-note.txt", "content": "mine"}, headers=owner)

    for method, url, kwargs in [
        ("get", f"/api/v1/projects/{pid}/files", {}),
        ("post", f"/api/v1/projects/{pid}/files/file", {"json": {"parent_path": "", "name": "x.txt"}}),
        ("delete", f"/api/v1/projects/{pid}/files/secret-note.txt", {}),
        ("post", f"/api/v1/projects/{pid}/files/upload", {"files": {"files": ("y.txt", io.BytesIO(b"y"), "text/plain")}}),
    ]:
...
async def test_waitlist_rate_limit_enforced(client):
    last = None
    for i in range(7):
        last = await client.post("/api/v1/waitlist", json={"email": f"bulk{i}@example.com"})
...
async def test_login_rate_limit_enforced(client):
    last = None
    for _ in range(12):
        last = await client.post("/api/v1/auth/login", json={"email": "nope@example.com", "password": "bad"})
        if last.status_code == 429:
            break
