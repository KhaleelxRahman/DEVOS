...
async def connect(current_user: User = Depends(get_current_user)):
    ...
    return ApiResponse(success=True, data={"authorization_url": GitHubService.authorization_url(state)})
...
async def callback(
    ...
    try:
        payload = jwt.decode(state, settings.AUTH_SECRET, algorithms=[settings.JWT_ALGORITHM])
    ...
    await GitHubService.save_connection(db, payload["sub"], github_user, token_payload["access_token"])
    return RedirectResponse(url=f"{settings.FRONTEND_APP_URL.rstrip('/')}/app/settings", status_code=303)
