...
SECRET_KEY_PATTERNS = [
    re.compile(
        r"(?i)(api[_-]?key|secret|token|password|auth[_-]?key)\s*[:=]\s*['\"]?([a-zA-Z0-9_\-\.]{8,})['\"]?"
    ),
    ...
    async def build_project_context(
        ...
        for readme_name in ("README.md", "readme.md", "README"):
            try:
                ...
                readme_excerpt = ContextService.sanitize_text(readme_content.content)[
                    :4000
                ]
                break
            except Exception:
        ...
        if current_file:
            try:
                active_file_data = FileService.get_file_content(
                    project_id, current_file
                )
                active_file_data.content = ContextService.sanitize_text(
                    active_file_data.content
                )
            except Exception:
                active_file_data = None
        ...
        try:
            ...
            for field in ("modified", "added", "deleted", "untracked"):
                setattr(
                    ...
                    [
                        f
                        for f in getattr(git_status, field)
