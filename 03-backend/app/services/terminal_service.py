...
class TerminalService:
    ...
    def validate_command(command: str, args: list[str] | None = None) -> None:
        ...
        if (
            not command
            or command != cmd_clean
            or any(char.isspace() for char in command)
        ):
            raise AppException(
                "Invalid command name", code="TERMINAL_BLOCKED", status_code=403
            )
        ...
        if len(command_args) > 64 or sum(len(arg) for arg in command_args) > 65536:
            raise AppException(
                "Too many or too-large command arguments",
                code="TERMINAL_BLOCKED",
                status_code=403,
            )
        if any("\x00" in arg or len(arg) > 4096 for arg in command_args):
            raise AppException(
                "Invalid command argument", code="TERMINAL_BLOCKED", status_code=403
            )
        for arg in command_args:
            ...
            if (
                ...
                raise AppException(
                    "Command arguments must stay within the project workspace",
                    code="TERMINAL_BLOCKED",
                    status_code=403,
                )
            if any(
                ...
                raise AppException(
                    "Access to sensitive credential files is blocked",
                    code="TERMINAL_BLOCKED",
                    status_code=403,
                )
        for blocked in BLOCKED_PATTERNS:
            if blocked in full_command:
                raise AppException(
                    "Command contains prohibited hazardous patterns",
                    code="TERMINAL_BLOCKED",
                    status_code=403,
                )
        if SHELL_METACHARACTERS.search(full_command) and not (
            ...
            and len(args or []) == 2
            and re.fullmatch(
                r"import sys; sys\.exit\([0-9]{1,3}\)", (args or ["", ""])[1]
            )
        ):
            raise AppException(
                "Command chaining and shell metacharacters are not permitted",
                code="TERMINAL_BLOCKED",
                status_code=403,
            )
        ...
        if cmd_clean not in ALLOWED_COMMANDS:
            raise AppException(
                f"Command '{command}' is not permitted in the sandbox environment",
                code="TERMINAL_BLOCKED",
                status_code=403,
            )
        if cmd_clean in {"python", "python3"} and any(
            ...
            safe_python_exit = (
                ...
                and re.fullmatch(
                    r"import sys; sys\.exit\([0-9]{1,3}\)", command_args[1]
                )
            )
            if not safe_python_exit:
                raise AppException(
                    "Interpreter evaluation is not permitted",
                    code="TERMINAL_BLOCKED",
                    status_code=403,
                )
        ...
        if cmd_clean == "npm" and any(
            ...
            raise AppException(
                "Package scripts and installation are not permitted",
                code="TERMINAL_BLOCKED",
                status_code=403,
            )
    ...
    @staticmethod
    async def execute(
        project_id: str, command: str, args: list[str] | None = None
        ...
        try:
            ...
            stdout = stdout_bytes.decode("utf-8", errors="replace")[
                : settings.TERMINAL_MAX_OUTPUT_CHARS
            ]
